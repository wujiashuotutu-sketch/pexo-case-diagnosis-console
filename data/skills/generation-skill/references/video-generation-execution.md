...
| **Grok** | Built-in audio (pure text only) | Always generates audio. |
...
### Pre-Planned Frame Chain (parallel seamless junctions)

**When:** `frame_chain_mode: pre_planned` from script-skill (intent-first; **≥2 segments**, any duration).

**Two phases, two batches:**

1. **Phase A — stills:** One `image_generate` batch (GPT Image 2 preferred) producing all `chain_nodes` and optional `MID_*` ids listed in `slot_node_ids`. Output **`frame_chain_manifest`** (`node_id → asset://`).
2. **Phase B — video:** One parallel `video_generate` batch. For each row in `frame_chain_plan.per_sequence_slots`:
   - `image_list[k]` = `frame_chain_manifest[slot_node_ids[k]]` (same order; no reordering).
   - Prompt: `"image 1"` binds to `slot_node_ids[0]` (entry); `"image {N}"` binds to `slot_node_ids[-1]` (exit); describe journey from image 1 to image N. API has no `first_frame` / `last_frame` fields.

**2-segment minimum chain:** `OPEN_S1` → `END_S1` where `END_S1` is seq1 last slot and seq2 first slot.

**High in-segment density:** Add `MID_*` between first and last without changing cross-segment slots. Cap by model image limit (Seedance ≤9); trim MID before trimming chain slots.

**Do not** wait for Phase B output to supply junction stills when mode is `pre_planned_stills`.

### Junction link: tail-frame extraction (alternate / repair)

**When:** `junction_link_mode: tail_extract_junction`, user demands pixel match to generated output, or pre-planned junction failed QA.

Extract static PNG/JPEG from final ~0.5s of completed upstream segment; assign as downstream `image_list[0]`; regenerate downstream only. This path may **partially serialize** affected junctions — state tradeoff to user when they choose it over pre-planned stills.
...
**Subject Coverage Manifest**: For any recurring visible character/person/product, maintain a compact manifest with `subject_id`, `visible_in_sequences`, `required_ref_type`, `ref_asset`, and per-sequence payload expectations. Every sequence that shows that subject must include the same `ref_asset` in `image_list` or `video_list` and bind it in the prompt. Prompt text like "same man", "same presenter", or "same product" is not a substitute for payload coverage.

**Late reference invalidation**: If a `character_ref`, `product_ref`, or identity-critical key frame is generated after any sequence has already been produced, all existing sequences containing that subject but generated without the new reference become stale. Regenerate those stale sequences with the reference before assembly; do not assemble anchored and unanchored versions together.
...
| Grok | `@filename` | `"@style.jpg for art style reference"` |
...
1. **Reference material mapping** - Declare which references are used and what role each plays. Use the model-appropriate syntax (positional `"image 1"` for Seedance, `@filename` for Grok, `<filename>` for Kling). Declare the element being referenced and its role (product_ref, character_ref, style_ref, etc.).
...
- **Payload proof is required**: The reference must appear in the actual tool payload (`image_list` / `video_list`), not merely in the agent's context or prompt memory.
...
- **Prompt text is supplemental only**: Repeating age, clothing, ethnicity, brand traits, or "same person" cannot satisfy a required `character_ref` / `product_ref` gate.
