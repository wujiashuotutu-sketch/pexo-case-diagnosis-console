...
The plan must express this as a structure downstream tools can check, not only prose. For every spoken line, include:

```yaml
voice_source_declaration:
  - line_id: line_01
    sequence_id: seq1
    speaker_id: presenter_01
    voice_identity_id: presenter_01_voice
    speaker_visibility: on_camera
    planned_source: audio_list_tts
    exact_text: "..."
    requires_tts_asset_before_video: true
    requires_visual_reference_for_audio_list: true
    generation_payload_requirements:
      audio_list_must_include_line_asset: true
      visual_reference_must_be_present: true
    assembly_guard: no_separate_vo_for_same_line
```

If `requires_visual_reference_for_audio_list` is true but the brief is pure text with no user image/video reference, the plan must choose a concrete route before generation: create/derive a visual reference, use exact quoted co-generation if voice continuity permits, or rewrite as disclosed silent B-roll. Do not leave this to generation-skill to infer.
