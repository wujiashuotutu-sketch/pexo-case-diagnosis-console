...
For videos with voiceover/narration, the VO audio is the spoken-completeness anchor. **VO duration is non-compressible, but visual quality and the user's target duration are also real constraints.** Visual duration can be adjusted only when the result still preserves the intended pacing, shot design, and viewing effect. The timeline's primary axis is determined by whichever element is immutable, but conflicts must be replanned rather than hidden in the edit.
...
3. **Plan the timeline based on VO duration and visual effect together** - video segment `out_ms`, `start_ms`, transition timing, and BGM duration align to the VO rhythm when that preserves the shot design. If the measured VO requires unnatural slow motion, repeated/static filler, or a large duration drift, stop and replan instead of forcing the current footage to fit.
...
2. **Generate additional or replacement visual beats** when the VO content is important and the current footage would need obvious slow motion or padding.
3. **Extend or retime the visual plan** if the user allows a longer video or if the target duration has slack and the pacing still works.
4. **Split the narration across a longer structure** when the information density is too high for one segment.
5. **Ask the user to choose** when the hard duration, required spoken content, and visual quality cannot all be preserved.
...
Do not solve VO overflow by substantially slowing the existing video either. Playback speed changes are timing polish, not the primary repair. Any required video speed below `0.85` or above `1.15`, total duration drift over 10% from a hard/communicated target, or visible pacing degradation triggers strategy adjustment: shorter regenerated VO, more visual generation, target-duration discussion, or user choice.
