...
description: "Four-layer model routing hierarchy - Grok (L1 fast pure-text preview/NSFW), Seedance Fast (L2 reference-capable fast visual anchor), Seedance (L3 production default), Kling (L4 precision single-frame/key-frame production and fallback). Covers routing decision tree, tool call mapping, per-model usage scenarios and capacity limits, fallback & degradation chains, and user-facing communication rules."
---
...
|-------|-------|-----------|---------|-------------|-------------|
| L1 Fast Preview | Grok | `fal` | `xai/grok-imagine-video/text-to-video` | Fast & wild | Pure-text non-final preview without precision/reference needs; NSFW content; user explicitly wants speed |
...
- **Has reference materials** (images, video, audio):
  ...
  - **Grok is eliminated** - Grok only supports `text2video` with no `image_list`/`video_list`/`audio_list`. Selecting Grok when references exist silently discards all user-uploaded materials. This applies to both production and preview.
- **Pure text** (no reference materials):
  - See Grok vs Seedance routing below
...
**Step 3 - Grok vs Seedance Fast routing (preview / visual anchor)**:
...
2. **Grok only when any of these conditions is met**:
   ...
   - **NSFW content**: Seedance's content moderation is stricter; route NSFW to Grok.
3. **User intent overrides all**: User explicitly wants speed -> Grok. User explicitly wants quality -> Seedance Fast. Regardless of other conditions.
...
Grok (`fal` / `xai/grok-imagine-video/text-to-video`) - text-led generation with built-in audio. L1 positioning - used for fast preview and NSFW content.
...
**Usage**: Pure text-to-video only - **no `image_list`, `video_list`, or `audio_list` support**. When user-uploaded reference materials exist, Grok must NOT be selected - choosing Grok silently discards all references. Use Seedance or Kling with `reference2video` mode instead.

**Audio**: Built-in - generates audio alongside visuals by default.

**Prompt**: Single flowing narrative - mood, visual style, key moments, audio atmosphere. Use `@filename` syntax if material references are available.
...
2. For pure T2V (no references) -> fall back to **Grok** for the failing segment only.
3. For reference-based -> retry Kling with simplified prompt. Do **not** fall back to Grok when references are present.
...
- Model names (Kling, Grok, Seedance, Seedream)
