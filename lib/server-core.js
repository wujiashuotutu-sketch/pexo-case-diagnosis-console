...
function buildSkillsTree(caseId) {
  ...
  for (const f of traceFiles) {
    ...
    for (const o of (trace.observations || [])) {
      ...
      if (!skillReadMap.has(skillName)) skillReadMap.set(skillName, { skillRanges: [], refRanges: new Map(), traceFiles: new Set(), firstTraceIdx: traceFiles.indexOf(f) });
  ...
  for (const sn of skillNodes) { if (!skillReadMap.has(sn.skillName)) skillReadMap.set(sn.skillName, { skillRanges: [], refRanges: new Map(), traceFiles: new Set(), firstTraceIdx: 999 }); }
  ...
  for (const [skillName, data] of skillReadMap) {
    ...
    tree.push({ skill: skillName, refs, allRefCount: allRefFiles.length + 1, readRefCount: readRefNames.size + (skillMdReads.length > 0 ? 1 : 0), aggReadLines, aggTotalLines, aggPct: aggTotalLines > 0 ? Math.round(aggReadLines / aggTotalLines * 100) : 0, _order: data.firstTraceIdx });
  ...
  tree.sort((a, b) => a._order - b._order);
  for (const t of tree) delete t._order;
