...
const EXTERNAL_CASES_DIR = process.env.EXTERNAL_CASES_DIR || resolve(PROJECT_ROOT, "..", "case-diagnosis-data", "cases");
...
function findCaseDir(caseId) {
  ...
  if (existsSync(EXTERNAL_CASES_DIR)) {
    const extPath = join(EXTERNAL_CASES_DIR, caseId);
    if (existsSync(extPath) && countTraces(extPath) > 0) return extPath;
  }
...
const BUNDLED_SKILLS_DIR = join(PROJECT_ROOT, "data", "skills");

function readLocalSkillFile(skillName, relPath) {
  try { return readFileSync(join(BUNDLED_SKILLS_DIR, skillName, relPath), "utf8"); } catch { return null; }
}

function listLocalSkillRefs(skillName) {
  try { return readdirSync(join(BUNDLED_SKILLS_DIR, skillName, "references")).filter(f => f.endsWith(".md")); } catch { return []; }
}

function buildSkillsTree(caseId) {
  ...
  const skillReadMap = new Map();
  const traceUserMessages = new Map();

  for (const f of traceFiles) {
    const trace = readJson(join(caseDir, f));
    if (!trace) continue;
    const msgs = trace.input?.messages || [];
    for (const m of msgs) { if (m.type === "human" && m.content) { traceUserMessages.set(f, String(m.content).slice(0, 200)); break; } }
    for (const o of (trace.observations || [])) {
      if (o.name !== "read_file") continue;
      const inp = o.input || {};
      let fp = "";
      if (typeof inp === "object" && inp !== null) fp = inp.file_path || "";
      else if (typeof inp === "string") { const m2 = inp.match(/file_path['":\s]+['"]([^'"]+)['"]/); if (m2) fp = m2[1]; }
      if (!fp) continue;
      const isSkill = fp.includes("SKILL.md"), isRef = fp.toLowerCase().includes("reference");
      if (!isSkill && !isRef) continue;
      const skillName = isSkill ? fp.match(/\/([^/]+)\/SKILL\.md/)?.[1] : fp.match(/\/([^/]+)\/references\//)?.[1];
      if (!skillName) continue;
      const content = typeof o.output === "string" ? o.output : (o.output?.content || "");
      let readStart = null, readEnd = null;
      for (const cl of content.split("\n")) { const lm = cl.match(/^\s*(\d+)\t/); if (lm) { const ln = parseInt(lm[1], 10); if (readStart === null || ln < readStart) readStart = ln; if (readEnd === null || ln > readEnd) readEnd = ln; } }
      if (!skillReadMap.has(skillName)) skillReadMap.set(skillName, { skillRanges: [], refRanges: new Map(), traceFiles: new Set(), firstSeenIdx: traceFiles.indexOf(f) });
      const entry = skillReadMap.get(skillName); entry.traceFiles.add(f);
      if (isSkill && readStart !== null) entry.skillRanges.push({ start: readStart, end: readEnd });
      else if (isRef && readStart !== null) { const refFile = fp.split("/").pop(); if (!entry.refRanges.has(refFile)) entry.refRanges.set(refFile, []); entry.refRanges.get(refFile).push({ start: readStart, end: readEnd }); }
    }
  }

  const skillNodes = nodes.filter(n => n.type === "skill_branch");
  for (const sn of skillNodes) { if (!skillReadMap.has(sn.skillName)) skillReadMap.set(sn.skillName, { skillRanges: [], refRanges: new Map(), traceFiles: new Set(), firstSeenIdx: 999 }); }

  const tree = []; const roundsMap = new Map();
  for (const [skillName, data] of skillReadMap) {
    const refs = [];
    const skillContent = readLocalSkillFile(skillName, "SKILL.md");
    const skillTotalLines = skillContent ? skillContent.split("\n").length : null;
    const skillMdReads = mergeLineRanges(data.skillRanges);
    const skillMdReadCount = skillMdReads.reduce((s, r) => s + (r.end - r.start + 1), 0);
    refs.push({ path: "SKILL.md", label: "SKILL.md", totalLines: skillTotalLines, readRanges: skillMdReads, readLineCount: Math.min(skillMdReadCount, skillTotalLines || Infinity), fullContent: skillContent, wasRead: skillMdReads.length > 0, isSkillFile: true });

    const allRefFiles = listLocalSkillRefs(skillName);
    const readRefNames = new Set(data.refRanges.keys());
    for (const refFile of allRefFiles) {
      const refContent = readLocalSkillFile(skillName, `references/${refFile}`);
      const refTotalLines = refContent ? refContent.split("\n").length : null;
      const ranges = data.refRanges.get(refFile) || [];
      const mr = mergeLineRanges(ranges);
      const rlc = Math.min(mr.reduce((s, r) => s + (r.end - r.start + 1), 0), refTotalLines || Infinity);
      const wasRead = readRefNames.has(refFile);
      refs.push({ path: `references/${refFile}`, label: refFile, totalLines: refTotalLines, readRanges: mr, readLineCount: wasRead ? rlc : 0, fullContent: refContent, wasRead });
    }

    let aggReadLines = 0, aggTotalLines = 0;
    for (const r of refs) { if (r.totalLines) { aggTotalLines += r.totalLines; aggReadLines += r.readLineCount; } }
    tree.push({ skill: skillName, refs, allRefCount: allRefFiles.length + 1, readRefCount: readRefNames.size + (skillMdReads.length > 0 ? 1 : 0), aggReadLines, aggTotalLines, aggPct: aggTotalLines > 0 ? Math.round(aggReadLines / aggTotalLines * 100) : 0 });
    for (const tf of data.traceFiles) { if (!roundsMap.has(tf)) roundsMap.set(tf, { traceFile: tf, skills: new Set() }); roundsMap.get(tf).skills.add(skillName); }
  }

  const rounds = []; let roundIdx = 0;
  for (const f of traceFiles) { const rd = roundsMap.get(f); if (!rd || rd.skills.size === 0) continue; roundIdx++; rounds.push({ round: roundIdx, traceFile: f, userMessage: traceUserMessages.get(f) || "", skills: [...rd.skills].map(sn => { const full = tree.find(t => t.skill === sn); return { skill: sn, aggPct: full?.aggPct ?? 0, aggReadLines: full?.aggReadLines ?? 0, readRefCount: full?.readRefCount ?? 0, allRefCount: full?.allRefCount ?? 0 }; }) }); }

  const skillFirstTs = new Map();
  for (const sn of skillNodes) { const ts = sn.timestamp || ""; if (!skillFirstTs.has(sn.skillName) || ts < skillFirstTs.get(sn.skillName)) skillFirstTs.set(sn.skillName, ts); }
  tree.sort((a, b) => (skillFirstTs.get(a.skill) || "z").localeCompare(skillFirstTs.get(b.skill) || "z"));

  return { caseId, tree, rounds };
}
...
export function listCases() {
  const cases = new Map();
  for (const dir of [BUNDLED_CASES_DIR, EXTERNAL_CASES_DIR, TMP_CASES_DIR]) {
    ...
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      cases.set(entry.name, {
        ...
        source: dir === BUNDLED_CASES_DIR ? "bundled" : dir === EXTERNAL_CASES_DIR ? "external" : "fetched",
      });
    }
  }
...
export async function fetchFromLangfuse(caseId) {
  ...
  async function fetchTracesByParam(paramName, paramValue) {
    let page = 1;
    while (page <= 10) {
      const url = `${host}/api/public/traces?${paramName}=${encodeURIComponent(paramValue)}&page=${page}&limit=50`;
      const resp = await fetch(url, { headers });
      if (!resp.ok) throw new Error(`Langfuse API error: ${resp.status}`);
      const data = await resp.json();
      const traces = data.data || [];
      for (const trace of traces) {
        const obsResp = await fetch(`${host}/api/public/observations?traceId=${trace.id}&limit=200`, { headers });
        const obsData = obsResp.ok ? await obsResp.json() : { data: [] };
        trace.observations = obsData.data || [];
        traceIdx++;
        writeFileSync(join(caseDir, `trace-${traceIdx}-${trace.id.slice(0, 8)}.json`), JSON.stringify(trace, null, 2));
      }
      if (!traces.length || !data.meta?.hasMore) break;
      page++;
    }
  }

  await fetchTracesByParam("sessionId", normalized);
  if (countTraces(caseDir) === 0) {
    await fetchTracesByParam("name", normalized);
  }
